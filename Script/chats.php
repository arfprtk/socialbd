...
if(locked($uid, $rid))
    {
      ...
      echo "<a href=\"index.php?action=chat&amp;sid=$sid&amp;type=send&amp;browse?start\">Chatrooms</a>";
      echo "</p>";
      echo "</body>";
      echo "</html>";
      exit();
 ...
 if(kick($uid, $rid))
    ...
    addtochat($uid, $rid);
       ...
       echo "<meta http-equiv=\"refresh\" content=\"20; URL= chats.php?action=$action&amp;sid=$sid&amp;rid=$rid\"/>";
        echo "<timer value=\"200\"/><p align=\"left\">";
        addonline($uid,"Chatrooms","");
        echo "<small>
        <a href=\"chats.php?action=say&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Options</a>/";
        $time = date('dmHis');
        echo "<a href=\"chats.php?time=$time&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Refresh</a><br/><br/>";
                 if(iscowner($uid, $rid))
             {
     if(!locked($uid, $rid))
    {

      echo "<a href=\"genproc.php?action=lock&amp;sid=$sid&amp;rid=$rid&amp;browse?start\">Lock</a><br/>";
    }else{
        echo "<a href=\"genproc.php?action=unlock&amp;sid=$sid&amp;rid=$rid&amp;browse?start\">Unlock</a><br/>";
    }
}
...
$winner2 =$rpgds[1];
 ...
 $time = mysql_fetch_array(mysql_query("SELECT MAX(displaytime) FROM dcroxx_me_quizrooms ORDER by id LIMIT 1"));
  ...
  if($pmfl<$tm)
  {
   mysql_query("INSERT INTO dcroxx_me_chat SET  chatter='133', who='', timesent='".time()."', msgtext='Nobody got the correct answer. Next question.', rid='".$rid."';");
...
echo " Time Left to answer:$rema seconds.REWARD: $kano Plusses and GP.";
}
}
}
    if($rid==182)
...
echo "<a href=\"chats.php?action=reset&amp;sid=$sid&amp;rid=182\">CLEAR DATA</a><br/>";
}
if($rid==454)
        {
...
mysql_query("INSERT INTO dcroxx_me_chat SET  chatter='133', timesent='".time()."', msgtext='$unick2 got $winner2 cards, and the 1000 plusses prize', perm='".$perm[0]."', rid='".$rid."';");
...
echo "<b>$by: </b>$text<br/>";
  echo "REPLY:<br/>";
 echo "<form action=\"chats.php?action=sendpm&amp;who=$unreadinbox[0]&amp;sid=$sid&amp;rid=$rid\" method=\"post\">";
  echo "<input name=\"pmtext\" maxlength=\"500\"/><br/>";
  echo "<input type=\"submit\" value=\"Send\"/>";
  echo "</form>";
echo "<br/><a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw&amp;cancel=1&amp;idn=$unreadinbox[2]&amp;time=$time\">Cancel</a>";
   }
...
echo "Jack en Poy challenge from $by BET: $ncl[1]<br/>";
 echo "<form action=\"chats.php?action=jnp&amp;sid=$sid&amp;rid=$rid&amp;time=$time\" method=\"post\">";
...
echo "</form>";
      ...
      echo "</small></p>";
        ...
        if (trim($message) != "")
        {
          $nosm = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM dcroxx_me_chat WHERE msgtext='".$message."'"));
          if($nosm[0]==0){
            $perm = mysql_fetch_array(mysql_query("SELECT perm, id FROM dcroxx_me_users WHERE perm>'0' AND id='".$uid."'"));
               */
...
$tid = correct($message,$question3);
  if($tid==0)
    {
 $by = getnick_uid($uid);
 mysql_query("INSERT INTO dcroxx_me_chat SET  chatter='133', timesent='".time()."', msgtext='$by sorry your answer is incorrect.Please try again', rid='".$rid."'");
...
mysql_query("INSERT INTO dcroxx_me_quizrooms SET  displaytime='".time()."', question='".$winner."', answer='".$winner2."';");
 ...
 $by = getnick_uid($uid);
    mysql_query("INSERT INTO dcroxx_me_chat SET  chatter='133', who='".$who."', timesent='".time()."', msgtext='$by you are correct. [b]$message [/b] you got $kano plusses. Next question', rid='".$rid."'");
...
$chnick = getnick_uid($chat[0]);
                   ...
                   echo "<br/><a href=\"chats.php?action=say2&amp;sid=$sid&amp;who=$chat[0]&amp;rid=$rid&amp;rpw=$rpw\">$optlink$chat[5]</a>:$tosay";
                  }
                }
        ...
        echo "<p align=\"left\">";
echo "<a href=\"chats.php?action=say&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Write</a><br/>";
        echo "<a href=\"chats.php?time=";
        echo date('dmHis');
        echo "&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw";
        echo "\">Refresh</a>";
	if(ismod($uid))
  {
echo "<a href=\"lists.php?action=kicklist&amp;sid=$sid&amp;rid=$rid\">----Kick List</a>";
}
        $chatters=mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM dcroxx_me_chonline where rid='".$rid."'"));
        echo "<br/><a href=\"chats.php?action=inside&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Who's Inside($chatters[0])</a><br/>";
...
/////////////////////////////////////////////////////SAY
        ...
        addonline($uid,"Chatroom Options","");
            ...
            echo "<form action=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw\" method=\"post\">";
            ...
            echo "<small><a href=\"chats.php?action=smilies&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#187;Smilies</a></small><br/>";
            echo "<small><a href=\"chats.php?action=inside&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#187;Who's Inside</a></small><br/>";
            echo "<small><a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#171;Chatroom</a></small></p>";
        //end
        ...
        addonline($uid,"Writing Chat Message","");
            ...
            //echo "<small><a href=\"lists.php?action=chmood&amp;sid=$sid&amp;page=1\">&#187;Chat mood</a></small><br/>";
            //echo "<small><a href=\"chats.php?action=inside&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#187;Who's Inside</a></small><br/>";
            echo "<small><a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#171;Back to room</a></small></p>";
        //end
    ...
    /////////////////////////////////////////////////////SAY2
        ...
        addonline($uid,"Writing chat message","");
         if($rid==182)
        {
 echo "<form action=\"chats.php?action=jnpc&amp;sid=$sid&amp;rid=$rid&amp;time=$time\" method=\"post\">";
    echo "JNP Challenge: <select name=\"kamay\">";
  echo "<option value=\"1\">Paper</option>";
  echo "<option value=\"2\">Scissors</option>";
  echo "<option value=\"3\">Rock</option>";
...
echo "</form>";
            ...
            echo "<small><a href=\"chats.php?action=expose&amp;sid=$sid&amp;who=$who&amp;rid=$rid&amp;rpw=$rpw\">&#187;Expose $unick</a></small><br/>";
                    if(iscowner($uid, $rid))
             {
     if(!kick($who, $rid))
    {

      echo "<a href=\"genproc.php?action=kick&amp;sid=$sid&amp;who=$who&amp;rid=$rid\">&#187;Kick</a><br/>";
    }else{
        echo "<a href=\"genproc.php?action=unkick&amp;sid=$sid&amp;who=$who&amp;rid=$rid\">&#187;Unkick</a><br/>";
    }
}
...
$idle2 = "- idle:";
          ...
          $userl = "<small><a href=\"chats.php?action=say2&amp;sid=$sid&amp;who=$ins[1]&amp;rid=$rid&amp;rpw=$rpw\">$unick</a> $idle2 $idle<br/></small>";
          echo "$userl";
        }
        echo "<br/><br/>";
        echo "<a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#171;Back to room</a><br/>";
...
else if($action=="smilies")
{
    ...
    echo "<small><a href=\"chats.php?action=options&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Options</a>, ";
    echo "<a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Chatroom</a><br/></small>";
    //////ALL LISTS SCRIPT <<
    ...
    while ($item = mysql_fetch_array($items))
   {
    ...
    {
      $ppage = $page-1;
      echo "<a href=\"lists.php?action=smilies&amp;page=$ppage&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">&#171;PREV</a> ";
    }
    if($page<$num_pages)
    {
      $npage = $page+1;
      echo "<a href=\"chats.php?action=smilies&amp;page=$npage&amp;sid=$sid&amp;rid=$rid&amp;rpw=$rpw\">Next&#187;</a>";
    }
    ...
    if($num_pages>2)
    {
      $rets = "Jump to page<input name=\"pg\" format=\"*N\" size=\"3\"/>";
        $rets .= "<anchor>[GO]";
        $rets .= "<go href=\"chats.php\" method=\"get\">";
 ...
 if($gpst[0]>=$bet)
{
 ...
 echo "<a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw&amp;type=send&amp;time=$time\">back to room</a><br/>";
  echo "</p>";
  echo "</body>";
}
  else if ($action=="jnp")
...
echo "<br/><a href=\"chats.php?sid=$sid&amp;rid=$rid&amp;rpw=$rpw&amp;type=send&amp;time=$time\">back to room</a><br/>";
  echo "</p>";
  echo "</body>";
}
