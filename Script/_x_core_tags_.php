...
$bago = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM ibwfrr_gconline WHERE uid='".$uid."'"));
if($bago[0]==0){
mysql_query("INSERT INTO ibwfrr_gconline SET lton='".time()."', uid='".$uid."'");
}else{
mysql_query("UPDATE ibwfrr_gconline SET lton='".time()."' WHERE uid='".$uid."'");
}
}
...
$rform = "<small>$errl</small><br/>";
  ...
  $rform .= "<small>Password:</small> <br/><input type=\"password\" name=\"pwd\" format=\"*x\" maxlength=\"30\"/><br/>";
...
//  $rform .= "<option value=\"2\">Sinhala</option>";
 ...
 //   $rform .= "<option value=\"4\">Indonesian</option>";
  ...
  $rform .= "<input type=\"text\" name=\"vercode\" /><br>\n";
    $r = $_GET["r"];
  if($r>0){
  $rform .= "<input type=\"hidden\" name=\"reffer\" value=\"$r\"/>";
}
...
$exec = mysql_query("DELETE FROM ibwfrr_gconline WHERE lton<'".$timeout."'");
...
{
$shbox = "<center><small>";
$lshout = mysql_query("SELECT shout, shouter, id  FROM dcroxx_me_shouts ORDER BY shtime DESC LIMIT 1");
...
$shnick = getnick_uid($shots[1]);
  ...
  $bulb= "<img src=\"$avlink\" width=\"25\" height=\"30\" alt=\"Profile\"/></a>";
  }
...
$shcomm = mysql_fetch_assoc(mysql_query("SELECT COUNT(*) comm FROM ibwfrr_shcomments WHERE shoutid='".$shots[2]."'"));
$shout = $shcomm['comm'];
$counts = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM ibwfrr_like WHERE shoutid='".$shots[2]."'"));
$counts1 = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM ibwfrr_dislike WHERE shoutid='".$shots[2]."'"));
...
$shbox .= "<a href=\"attach.php?action=attach\">attach</a>, ";
...
//$words = explode(' ',$msg);
//$msg = implode('+',$words);
$plc = mysql_fetch_array(mysql_query("SELECT imageurl FROM ibwfrr_images WHERE code='".$cid."'"));
...
$str = str_replace(" ","",$str);
$str = strtolower($str);
$res = mysql_query("SELECT site FROM ibwfrr_blockedsite");
...
return "IP Country: <b>".$ranges["country"]."</b><br/>Flag: ".$flag2;
}
}
}
/*//////////////////////Function add user to online list :P
...
if($newtime>1800){
   /* if(islotto($uid)){
        mysql_query("UPDATE ibwfrr_users SET plustime='0', totaltime=totaltime+$limit, plusses=plusses+9 WHERE id='".$uid."'");
        $current = mysql_fetch_array(mysql_query("SELECT plusses FROM ibwfrr_users WHERE id='".$uid."'"));
	$res = mysql_query("INSERT INTO ibwf_notifications SET text='Congratulations you have been awarded with 9 plusses for staying online for 30 minutes straight and you currently have ".$current[0]." plusses.', byuid='3', touid='".$uid."', unread='1', timesent='".time()."'");
        //$res = mysql_query("INSERT INTO ibwfrr_private SET text='[b]NOTIFICATION[br/]:Plusses Booster:[/b][br/]You are have been Online for 30 minutes. You have recieved 15 Plusses [br/][br/][small][i]p.s: This is an automated PM[/i][/small]', byuid='3', touid='".$uid."', timesent='".time()."'"); 
    }else{*/
        mysql_query("UPDATE dcroxx_me_users SET plustime='0', totaltime=totaltime+$limit, plusses=plusses+3 WHERE id='".$uid."'"); 
       $current = mysql_fetch_array(mysql_query("SELECT plusses FROM dcroxx_me_users WHERE id='".$uid."'"));
...
30 minutes straight and currently have ".$current[0]." plusses.', uid='".$uid."', time='".time()."'");  

$note = "Congratulations! [user=$uid][b]".getnick_uid($uid)."[/b][/user] have been awarded with 3 plusses for staying online for 30 minutes straight and currently have ".$current[0]." plusses.";
notify($note,$uid,$uid);	   
       // $res = mysql_query("INSERT INTO ibwfrr_private SET text='[b]NOTIFICATION:[/b][br/]You are have been Online for 30 minutes. You have recieved 5 Plusses [br/][br/][small][i]p.s: This is an automated PM[/i][/small]', byuid='3', touid='".$uid."', timesent='".time()."'"); 
   // }
}else{ 
...
$noi = mysql_fetch_array(mysql_query("SELECT count(*) FROM ibwfrr_country WHERE country='".getisp2(getip())."'"));
        if($noi[0]==0){
            mysql_query("INSERT INTO ibwfrr_country SET country='".getisp2(getip())."', lastupdate='".time()."'");
        }else{
            mysql_query("UPDATE ibwfrr_country SET lastupdate='".time()."' WHERE country='".getisp2(getip())."'");
        }
	 ...
	 if($reffer=="")
      {
      	...
      	mysql_query("INSERT INTO ibwfrr_invite SET invite='".$uid."', invitedby='".$reffer."', regdate='".time()."'");
...
$text = preg_replace(array('/(?i)\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))/', '/(^|[^a-z0-9_])@([a-z0-9_]+)/i', '/(^|[^a-z0-9_])#([a-z0-9_]+)/i'), array('<a href="$1">$1</a>', '$1<a href="/$2">$2</a>', '$1<a href="genproc.php?action=search&tag=$2">#$2</a>'), $text);
...
//$text = preg_replace("/\\n/", "<br />", $text);
		...
		if($smiles) {
			foreach($smiles as $smile => $img) {
				$text = str_replace($smile, '<img src="emoticons/'.$img.'" height="28" width="28" />', $text);
			}
		} 
