...
if ($_POST['action'] == 'image') {
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            ...
            echo '  <img src="model/' . $handle->file_dst_name . '" /><br/>';
            $info = getimagesize($handle->file_dst_pathname);
            echo '  link to the file just uploaded: <a href="model/' . $handle->file_dst_name . '">' . $handle->file_dst_name . '</a><br/>';
            $imageurl = "model/$handle->file_dst_name";
            $reg = mysql_query("INSERT INTO dcroxx_me_model SET uid='".$uid."', imageurl='".$imageurl."', sex='".$userinfo[1]."', time='".(time() - $timeadjust)."'");
