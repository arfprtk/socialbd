...
if(isbanned($uid))
	...
	addonline(getuid_sid($sid)," War Zone","wz.php?action=$main");
...
alt=\"\"/>";
	...
	$rec=mysql_fetch_array(mysql_query("SELECT plusses FROM dcroxx_me_users WHERE id='".$uid."'"));
	$recb=mysql_fetch_array(mysql_query("SELECT plusses FROM dcroxx_me_users WHERE id='".$who."'"));
	echo"<br/>U Have $rec[0] Credits<br/>";
	echo"$whonick Has $recb[0] Credits<br/><br/>";
	
	echo "AMOUNT YOU WANT TO BET!!!<br/>";
	echo "Credits: ";
	echo "<form action=\"wz.php?action=wz&amp;who=$who\" method=\"post\">";
    echo "<input name=\"bet\" format=\"*N\" maxlength=\"10\"/><br/>";
...
echo "<img src=\"smilies/wz1.jpg\"
alt=\"*\"/>";
 echo "<br/><br/>Now U can Battle With an  User!<br/> This battle is between <b>Death and Life!!</b> If u Win U will Have Double Credits dat u Bet..<br/> U can Challenge any <b> ONLINE </b> User. <br/><i>plz note: There is no use Challenging an <u>Offline user</u></i>";
