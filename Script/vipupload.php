...
/////////////////////////////////Members List
  ...
  if ($_POST['action'] == 'image') {
      ...
      if ($handle->uploaded) {
         ...
         if ($handle->processed) {
              ...
              echo '  <img src="vip/' . $handle->file_dst_name . '" /><br/>';
              $info = getimagesize($handle->file_dst_pathname);
              echo '  link to the file just uploaded: <a href="vip/' . $handle->file_dst_name . '">' . $handle->file_dst_name . '</a><br/>';
              $imageurl = "vip/$handle->file_dst_name";
              $reg = mysql_query("INSERT INTO dcroxx_me_vipgallery SET uid='".$uid."', imageurl='".$imageurl."', sex='".$userinfo[1]."', time='".(time() - $timeadjust)."'");
            } else {
              // one error occured
  ...
  ////// UNTILL HERE >> 
    echo "<p align=\"center\">";
   echo "<br/><br/><a href=\"vipgal.php?action=main\">&#171;Back to Gallery</a><br/>";
