...
$res = mysql_query("UPDATE dcroxx_me_users SET browserm='".$ubr."', ipadd='".$uip."' WHERE id='".getuid_sid($sid)."'");
  ...
  addonline(getuid_sid($sid),"Shout Dislikers","");

 $pstyle = gettheme($sid);
      echo xhtmlhead("Dislikers",$pstyle);
$shid = $_GET['shid'];

if($page=="" || $page<=0) {$page=1;}
$count = mysql_fetch_assoc(mysql_query("SELECT COUNT(*) page FROM ibwfrr_dislike WHERE shoutid='".$shid."'"));
...
echo '<center><small><b>Users Who Dislikes This Shout</b><br/><br/>';
...
$results = mysql_query("SELECT * FROM ibwfrr_dislike WHERE shoutid='".$shid."' ORDER BY ltime ASC LIMIT $limit_start, $event_per_page");
